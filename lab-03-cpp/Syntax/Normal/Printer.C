...
{
  int oldi = _i_;
  if (oldi > 1) render(NORMAL__L_PAREN);

  _i_ = 1; p->expr_1->accept(this);
  _i_ = 2; p->expr_2->accept(this);

  if (oldi > 1) render(NORMAL__R_PAREN);
  _i_ = oldi;
}
...
{
  int oldi = _i_;
  if (oldi > 2) render(NORMAL__L_PAREN);

  render("succ");
  _i_ = 3; p->expr_->accept(this);

  if (oldi > 2) render(NORMAL__R_PAREN);
  _i_ = oldi;
}
...
{
  int oldi = _i_;
  if (oldi > 3) render(NORMAL__L_PAREN);

  _i_ = 3; p->expr_->accept(this);
  render('.');
  visitIdent(p->ident_);

  if (oldi > 3) render(NORMAL__R_PAREN);
  _i_ = oldi;
}
