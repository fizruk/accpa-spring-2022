...
evalStep expr =
  case expr of
    ...
    Application (Abstraction _type body) arg ->
    ...
    go expr =
      case expr of
        BoundVar n
          ...
          | otherwise -> expr
        FreeVar _ -> expr
        Abstraction typeOfArg body ->
          Abstraction typeOfArg (shiftFrom (k + 1) body)
  ...
  where
    go expr =
      case expr of
        ...
        FreeVar _ -> expr
        Abstraction typeOfArg body ->
          Abstraction typeOfArg (substitute (n + 1, shift s) body)
        Application t1 t2 -> Application (go t1) (go t2)
