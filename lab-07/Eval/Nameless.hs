...
evalStep expr =
  case expr of
    ...
    Head (List (t:_ts)) -> Just t
    Head t ->
      case evalStep t of
        Just t' -> Just (Head t')
        Nothing -> Nothing
    Tail (List (_t:ts)) -> Just (List ts)
    Tail t ->
      case evalStep t of
        Just t' -> Just (Tail t')
        Nothing -> Nothing

    IsEmpty (List []) -> Just ConstTrue
    IsEmpty (List (_:_)) -> Just ConstFalse
    IsEmpty t ->
      case evalStep t of
        Just t' -> Just (IsEmpty t')
        Nothing -> Nothing

    ConsList t (List ts) -> Just (List (t:ts))
    ConsList h t ->
      case evalStep t of
  ...
  where
    go expr =
      case expr of
        ...
        Variant l t -> Variant l (go t)
        Match t cases -> Match (go t) (map shiftCase cases)
        List ts -> List (map go ts)
